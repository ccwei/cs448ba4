...
\caption{the actual review form on the Venture lab platform.
...
\caption{Individual View.}
\label{fig:ind-view}
\end{figure}


In aggregate view as shown in Figure \ref{fig:aggregate-view}, the text feedbacks for the selected teams are shown separately categorized by their types from feedback capture grid. However, the list can be very long, so the view also provides links for quick navigating to each feedback type. 
...
multiples to navigate through the original review list on the right part. Ranked by average score, each original review item in the list contains the scores distribution of the quantitative feedback rubrics, and the feedback capture grid as also shown in the
original form (Figure \ref{fig:review-form}).
